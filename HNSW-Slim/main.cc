...
DEFINE_int64(min_indegree0, 8, "max indegree for each node on level 0.");
DEFINE_int64(min_indegree, 4, "min indegree for each node.");
...
int main(int argc, char **argv) {
  ...
  size_t min_indegree0 = FLAGS_min_indegree0;
  size_t min_indegree = FLAGS_min_indegree;
  ...
  if (solve_strategy == "hnsw") {
    ...
    strategy =
        new HnswSlimStrategy(source_path, query_path, index_path,
                             threshold_level, top_degree_percent0,
                             top_degree_percent, top_M0, low_m0, top_M, low_m,
                             100, tmp_suffix);
  } else if (solve_strategy == "hnsw_slimq") {
      strategy =
          new HnswSlimQStrategy(
            source_path, query_path, index_path, threshold_level,
            top_degree_percent0, top_degree_percent, top_M0, low_m0, top_M, low_m,
            100, tmp_suffix);
  } else if (solve_strategy == "hnsw_slimzero") {
      strategy = new HnswSlimZeroStrategy(
          source_path, query_path, index_path, threshold_level,
          top_degree_percent0, top_degree_percent, top_M0, low_m0, top_M, low_m,
          min_indegree0, min_indegree, tmp_suffix);
    ...
    std::cout << "['hnsw', 'hnsw_slim', 'bruteforce', 'hnsw-slimq', 'hnsw-slimzero']"
              << std::endl;
    return 1;
  }
