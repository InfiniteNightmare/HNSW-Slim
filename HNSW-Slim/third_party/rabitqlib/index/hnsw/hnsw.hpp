...
class HierarchicalNSW {
   public:
    ...
    float getDist(const float*, PID);
    std::vector<std::vector<std::pair<float, PID>>> search(
        const float*, size_t, size_t, size_t, size_t
    );

    std::vector<std::vector<std::pair<float, PID>>> searchKnn(const float* query_data, size_t num_query, size_t k, uint32_t ef);
   ...
   rabitqlib::SplitSingleQuery<float> get_query_wrapper(const float* data) {
       std::vector<float> rotated_query(padded_dim_);
      this->rotator_->rotate(data, rotated_query.data());
    ...
    void get_q_to_centroids(std::vector<float>& rotated_query, std::vector<float>& q_to_centroids) {
        // Preprocess - get the distance from query to all centroids
        q_to_centroids.reserve(num_cluster_);
    ...
    size_t dataSize() {
       return cur_element_count_ * (size_ex_data_ + size_bin_data_);
   }

    size_t indexSize() {
       size_t size = 0;
       size_t data_size_ = size_ex_data_ + size_bin_data_;
       size += cur_element_count_ * (size_data_per_element_ - data_size_ -
                                sizeof(hnswlib::labeltype)); // data_level0_memory_
       size += cur_element_count_ * sizeof(int);         // element_levels_


       std::cout << "vecsize\t" << size_data_per_element_ << " / " << data_size_ << std::endl;
       for (size_t i = 0; i < cur_element_count_; i++) {
           size += sizeof(char *) +
                   (element_levels_[i] > 0
                        ? (size_links_per_element_ * element_levels_[i] + 1)
                        : 0);
       }
...
//   private:
    ...
    size_t getBinDataSize() {
        return offsetExData_ - offsetBinData_;
    }
    size_t getExDataSize() {
        return size_data_per_element_ - offsetExData_;
    }
};
...
inline void HierarchicalNSW::load(const char* filename, MetricType metric_type_input) {
    ...
    index_size += num_cluster_ * padded_dim_ * sizeof(float);
    ...
    for (size_t i = 0; i < cur_element_count_; i++) {
        ...
        if (link_list_size == 0) {
            ...
            index_size += link_list_size;
            list_size += link_list_size;
    ...
    std::cout << "RaBitQ::hnsw index size\t" << index_size << ", " << list_size << std::endl;
...
float HierarchicalNSW::getDist(const float* queries, PID id) {
    std::vector<float> rotated_query(padded_dim_);
    this->rotator_->rotate(queries, rotated_query.data());
    ...
    get_full_est(q_to_centroids, query_wrapper, id, start_estimate_record);
    float est_dist = start_estimate_record.est_dist;
    float low_dist = start_estimate_record.low_dist;

    // Insert initial candidate.
    auto res = ResultRecord(est_dist, low_dist);
    return res.est_dist;

}

inline std::vector<std::vector<std::pair<float, PID>>> HierarchicalNSW::searchKnn(
    const float* queries, size_t num_query, size_t k, uint32_t ef) {
    set_ef(ef);
    std::vector<std::vector<std::pair<float, PID>>> results(num_query);
    for (uint i=0; i<num_query; i++) {
        std::vector<float> rotated_query(padded_dim_);
        this->rotator_->rotate(queries + (i * dim_), rotated_query.data());
        maxheap<std::pair<float, PID>> knn = search_knn(rotated_query.data(), k);
        while (knn.size()) {
            results[i].emplace_back(knn.top());
            knn.pop();
        }
