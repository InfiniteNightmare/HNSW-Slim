...
class HierarchicalNSWSlimZero : public AlgorithmInterface<dist_t> {
public:
  ...
  size_t min_indegree0_{8};          // min indegree on l0
  size_t min_indegree_{4};           // min indegree

  mutable std::atomic<long> metric_distance_computations{0};
  mutable std::atomic<long> metric_hops{0};

  HierarchicalNSWSlimZero(SpaceInterface<dist_t> *s) {}

  HierarchicalNSWSlimZero(SpaceInterface<dist_t> *s, const std::string &location,
                      bool nmslib = false, size_t max_elements = 0,
                      bool allow_replace_deleted = false) {
    loadIndex(location, s, max_elements);
  }

  HierarchicalNSWSlimZero(SpaceInterface<dist_t> *s, size_t max_elements,
                      ...
                      size_t min_indegree0 = 4, size_t min_indegree = 2,
    ...
    min_indegree0_ = min_indegree0;
    min_indegree_ = min_indegree;
  ...
  HierarchicalNSWSlimZero(HierarchicalNSW<dist_t> *hnsw) { convertFromHNSW(hnsw); }

  ~HierarchicalNSWSlimZero() { clear(); }

  void addPoint(const void *datapoint, labeltype label,
                bool replace_deleted = false) {
    throw std::runtime_error("HierarchicalNSWSlimZero does not support addPoint");
  }
  ...
  void PruneByHeuristic(HierarchicalNSW<dist_t> *hnsw,
                        ...
                        std::vector<std::vector<std::atomic_size_t>> &indegrees,
                        int level, size_t M, size_t M_rev) {
    new_neighbors.clear();

    std::vector<tableint> neighbors_to_lock;
    neighbors_to_lock.reserve(heap_size);
    for (size_t i = 0; i < heap_size; i++) {
      neighbors_to_lock.push_back(heap[i].second);
    }

    std::sort(neighbors_to_lock.begin(), neighbors_to_lock.end());
    for (auto id : neighbors_to_lock) {
      hnsw->link_list_locks_[id].lock();
    }

    std::vector<bool> bitmap(heap_size, false);
    for (size_t i = 0; i < heap_size; i++) {
      std::pair<dist_t, tableint> current_pair = heap[i];
      if (indegrees[current_pair.second][level] <= M_rev) {
        new_neighbors.emplace_back(current_pair.second);
        bitmap[i] = true;
        hnsw->link_list_locks_[current_pair.second].unlock();
      }
    }

    size_t reserve_count = new_neighbors.size();
    for (size_t i = 0; i < heap_size; i++) {
      if (bitmap[i])
        continue;
      std::pair<dist_t, tableint> current_pair = heap[i];
      if (new_neighbors.size() >= M) {
        indegrees[current_pair.second][level]--;
        hnsw->link_list_locks_[current_pair.second].unlock();
        continue;
      }
      ...
      for (size_t j = 0;
           j < reserve_count && heap[j].first <= current_pair.first; j++) {
        dist_t curdist =
  ...
  void convertFromHNSW(HierarchicalNSW<dist_t> *hnsw) {
    ...
    std::vector<std::vector<std::atomic_size_t>> degree_histogram(
        maxlevel_ + 1); // [0, maxM0_+1]
    std::vector<std::vector<std::atomic_size_t>> indegrees(cur_element_count_);
    std::vector<std::atomic_size_t> level_cnts(maxlevel_ + 1);
    for (auto &row : degree_histogram) {
      row = std::vector<std::atomic_size_t>(maxM0_);
    }
#pragma omp parallel for schedule(dynamic)
    for (tableint i = 0; i < cur_element_count_; i++) {
      levelsizeint element_level = hnsw->element_levels_[i];
      indegrees[i] = std::vector<std::atomic_size_t>(element_level + 1);
    }
#pragma omp parallel for schedule(dynamic)
    for (tableint i = 0; i < cur_element_count_; i++) {
      levelsizeint element_level = hnsw->element_levels_[i];
...
#pragma omp parallel for schedule(dynamic)
        for (int j = 0; j < size; j++) {
          indegrees[*data++][l].fetch_add(1, std::memory_order_relaxed);
        }
      }
...
#pragma omp parallel for schedule(dynamic)
      for (int j = 0; j < size; j++) {
        indegrees[*data++][0].fetch_add(1, std::memory_order_relaxed);
      }
    }
    size_t acc = 0;
...
#pragma omp parallel for schedule(static)
      for (int l = 0; l <= v_level; l++) {
        ...
        size_t M_rev = l == 0 ? min_indegree0_ : min_indegree_;
...
#pragma omp parallel for
        ...
        PruneByHeuristic(hnsw, heap, size, new_neighbors_by_level[v][l],
                         indegrees, l, M0, M_rev);
      }
    }
...
#pragma omp parallel for
          ...
          std::sort(heap, heap + size, compare_by_first);
          PruneByHeuristic(hnsw, heap, size, nbrs, limit);
...
#pragma omp parallel for
        ...
        PruneByHeuristic(hnsw, heap, heap_size, indegrees,
                         new_neighbors_by_level[v][l], l,
                         l == 0 ? min_indegree0_ : min_indegree_, M0);
      }
    }
