...
public:
  ...
  // IDs only (exclude offsets)
  size_t getNeighborIdSizeOnly() const {
    size_t s = 0;
    for (size_t i = 0; i < cur_element_count_; i++) {
      s += sizeof(tableint) * get_total_neighbor(i);
    }
    return s;
  }
  ...
  void PruneByHeuristic(HierarchicalNSW<dist_t> *hnsw,
...
#pragma omp parallel for
        for (size_t j = 0; j < size; j++) {
          ...
          dist_t dist = hnsw->fstdistfunc_(
              hnsw->getDataByInternalId(v),
              hnsw->getDataByInternalId(neighbor_id), hnsw->dist_func_param_);
...
#pragma omp parallel for
  ...
  std::vector<tableint> convertFromHNSWWithDiff(HierarchicalNSW<dist_t> *hnsw,
                               std::ostream &output,
                               bool to_add_data = false) {
...
#pragma omp parallel for
        ...
        if (l == threshold_level_) {
          memcpy(neighbors + total_neighbor, nbrs_prune,
                 size * sizeof(tableint));
          total_neighbor += size;
          if (total_neighbor >= std::numeric_limits<linklistsizeint>::max()) {
            throw std::runtime_error(
      ...
      *(char **)(element + offsetNeighbor_) = neighbors_ptr;
      ...
      if (label_lookup_[getExternalLabel(i)] != i) {
        changed_new_nodes[changed_new_nodes_count++] = i;
        free(prev_neighbors_ptr);
      } else if (prev_neighbors_size == 0 || prev_neighbors_ptr == nullptr) {
    ...
    for (size_t i = 0; i < changed_old_cnt; i++) {
      ...
      output.write(element, label_offset_);
  ...
  uint32_t genPatch(std::ostream &output,
                 size_t& old_written, size_t& new_written,
                 size_t limit, bool to_add = false) {
    size_t written = 0;
    uint32_t finished = 0;

    for (; ind_old_ < old_nodes_cnt_; ind_old_++) {
      old_written++;
      tableint v = changed_old_nodes_[ind_old_];
      writeBinaryPOD(output, v);
      char *element = elements_ + v * size_data_per_element_;
      output.write(element, label_offset_);
      unsigned int neighborsSize = get_neighbor_size(v);
      ...
      written += neighborsSize + sizeof(unsigned int) + label_offset_
                + sizeof(tableint);
      if (written >= limit) {
        return finished;
      }
    ...
    for (; ind_new_ < new_nodes_cnt_; ind_new_++) {
      new_written++;
      tableint v = changed_new_nodes_[ind_new_];
      writeBinaryPOD(output, v);
      char *element = elements_ + v * size_data_per_element_;
      output.write(element, offsetNeighbor_);
      ...
      if (to_add) {
        output.write(element + offsetData_, data_size_);
      }
      written += neighborsSize + sizeof(unsigned int) + offsetNeighbor_
                  + sizeof(tableint) + data_size_;
      if (written >= limit) {
        return finished;
      }
    }
  ...
  void convertFromHNSWWithDiff(HierarchicalNSW<dist_t> *hnsw,
                               size_t& changed_old_cnt,
                               size_t& changed_new_cnt) {
    size_t prev_element_count = cur_element_count_;
    cur_element_count_ = hnsw->cur_element_count;
    has_deleted_elements_ = hnsw->num_deleted_ > 0;
    maxlevel_ = hnsw->maxlevel_;
...
#pragma omp parallel for
      ...
      *(char **)(element + offsetNeighbor_) = neighbors_ptr;
      ...
      if (label_lookup_[getExternalLabel(i)] != i) {
        changed_new_nodes_[changed_new_nodes_count++] = i;
        free(prev_neighbors_ptr);
      } else if (prev_neighbors_size == 0 || prev_neighbors_ptr == nullptr) {
        if (i >= prev_element_count) {
          changed_new_nodes_[changed_new_nodes_count++] = i;
        } else {
          changed_old_nodes_[changed_old_nodes_count++] = i;
        }
      } else if (prev_neighbors_size != neighbors_size ||
                 memcmp(prev_neighbors_ptr, neighbors_ptr, neighbors_size) !=
                     0) {
      ...
      while (changed) {
        ...
        for (size_t i = 0; i < size; i++) {
          ...
          if (i + 1 < size) {
            ...
            _mm_prefetch(getDataByInternalId(*(data + i + 1)), _MM_HINT_T0);
          }
#endif
  ...
  patchFromStream(std::istream &in,
    ...
    for (size_t i = 0; i < changed_old_cnt + changed_new_cnt; ++i) {
      ...
      if (i < changed_old_cnt) {
        in.read(element, label_offset_);
      } else {
        in.read(element, offsetNeighbor_);
        memcpy(element + offsetData_, data_set[getExternalLabel(id)].data(),
               data_size_);
    ...
    std::string LOG_path = std::format(
        "../statistics/LOG/client_{}.txt", cur_element_count_);
  }

  void putVector(std::ostream &output,std::vector<tableint>& ids,
                 uint32_t& current_pos) {
    auto max_num = 1024 * 1024 * 200 / data_size_;
    size_t limit, if_final, write_num;
    if (max_num + current_pos >= ids.size()) {
      limit = ids.size();
      if_final = 1;
    } else {
      limit = max_num + current_pos;
      if_final = 0;
    }
    write_num = limit - current_pos;

    writeBinaryPOD(output, if_final);
    writeBinaryPOD(output, write_num);

    for (; current_pos < limit; ++current_pos) {
      writeBinaryPOD(output, ids[current_pos]);
      output.write(getDataByInternalId(ids[current_pos]),
                data_size_);
    }
  }
  ...
  bool getVectorFromBatch(std::istream &in) {
    ...
    readBinaryPOD(in, num_vec);

    tableint id = 0;
    for (uint32_t i=0; i<num_vec; i++) {
      readBinaryPOD(in, id);
      char *element = elements_ + id * size_data_per_element_;
      in.read(element + offsetData_, data_size_);
    }

    return (finished == 1);
  }

  void patchFromStream(std::istream &in, bool to_add = false) {
    size_t prev_id_limit = cur_element_count_;
    readBinaryPOD(in, cur_element_count_);
  ...
  patchFromStream(std::istream &in,
    ...
    for (size_t i = 0; i < changed_old_cnt + changed_new_cnt; ++i) {
      ...
      if (i < changed_old_cnt) {
        ...
        in.read(element, offsetNeighbor_);
        memcpy(element + offsetData_, new_data[getExternalLabel(id)].data(),
               data_size_);
        label_lookup_[getExternalLabel(id)] = id;
      }
