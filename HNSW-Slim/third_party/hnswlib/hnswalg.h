...
public:
  ...
  HierarchicalNSW(SpaceInterface<dist_t> *s, size_t max_elements, size_t M = 16,
                  ...
                  std::string branching_factor = "16", size_t random_seed = 100,
                  bool allow_replace_deleted = false)
    ...
    if (branching_factor == "e") {
      mult_ = 1 / log(M_E);
    } else if (branching_factor == "sqrt") {
      mult_ = 1 / log(sqrt(2.0) / (sqrt(2.0) - 1));
    } else {
      try {
        mult_ = 1 / log(std::stod(branching_factor));
      } catch (const std::invalid_argument &e) {
        throw std::runtime_error("Invalid branching factor: " +
        branching_factor);
      } catch (const std::out_of_range &e) {
        throw std::runtime_error("Branching factor out of range: " +
        branching_factor);
      }
    }
    revSize_ = 1.0 / mult_;
...
//                    if (candidate_id == 0) continue;
  ...
  size_t indexSize() {
    size_t size = 0;
    size += max_elements_ * (size_data_per_element_ - data_size_ -
                             sizeof(labeltype)); // data_level0_memory_
    size += max_elements_ * sizeof(int);         // element_levels_
