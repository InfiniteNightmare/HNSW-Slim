...
int64_t time_cost(const std::chrono::system_clock::time_point &st,
...
void ReadData(const std::string &file_path,
              std::vector<std::vector<T>> &results, uint32_t &num,
              uint32_t &dim) {
...
// Read .fvecs and .ivecs, first partal% data
template <typename T>
void ReadData(const std::string &file_path,
              std::vector<std::vector<T>> &results, uint32_t &num,
              uint32_t &dim, int partial, size_t& max_size) {
  std::ifstream in(file_path, std::ios::binary);
  if (!in.is_open()) {
  ...
  std::ios::pos_type ss = in.tellg();
  size_t fsize = (size_t)ss;
  uint32_t data_num = (uint32_t)(fsize / (dim + 1) / 4);
  max_size = data_num;
  if (partial == 100) {
    num = data_num;
  } else {
    num = data_num * partial / 100;
  }
  results.resize(num);
  for (uint32_t i = 0; i < num; ++i) {
    results[i].resize(dim);
...
// Read .fvecs and .ivecs
inline void ReadData(const std::string &file_path, uint32_t &dim) {
  std::ifstream in(file_path, std::ios::binary);
  if (!in.is_open()) {
    std::cout << "open file error" << std::endl;
    exit(-1);
...
// Read .fvecs and .ivecs
inline void ReadSize(const std::string &file_path, uint32_t &num,
              uint32_t &dim) {
  std::ifstream in(file_path, std::ios::binary);
  if (!in.is_open()) {
    std::cout << "open file error" << std::endl;
...
void removeThresholdLevelPart(std::string &str) {
  int underscoreCount = 0;
  size_t thirdUnderscorePos = std::string::npos;
  size_t j = str.size() - 1;

  while (j > 0 && !std::isdigit(str[j])) {
    --j;
  }
